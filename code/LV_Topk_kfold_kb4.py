    ...
    for top_k in top_k_gen_list:
        ...
        for fold, (train_index, test_index) in enumerate(kf.split(X)):
            ...
            for model_name, model in models.items():
                ...
                f.write(f"{model_name} Test Precision: {precision_test:.4f}\n")
                f.write(f"{model_name} Test Recall: {recall_test:.4f}\n")
                f.write(f"{model_name} Test F1-Score: {f1_test:.4f}\n")
                # Lưu kết quả
                fold_results.append({
                    'Top_k': top_k,
                    'Fold': fold + 1,
...
# Lưu kết quả trung bình vào file Excel
summary.to_excel('LV_Topk_kfold_results.xlsx')
